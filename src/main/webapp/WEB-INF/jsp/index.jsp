...
<head>
    ...
    <link href="${basePath}/resources/zheng-admin/plugins/malihu-custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css"
          rel="stylesheet"/>
    ...
    <style>
        ...
        .s-profile > a {
            background: url(${basePath}/resources/zheng-admin/images/zheng-upms.png) left top no-repeat;
        }
    </style>
</head>
<body>
<header id="header">
    <ul id="menu">
        ...
        <li id="logo" class="hidden-xs">
            ...
            <span id="system_title">员工管理系统</span>
        </li>
        <li class="pull-right">
            <ul class="hi-menu">
                <!-- 搜索 -->
                        ...
                        <%--<c:forEach var="upmsSystem" items="${upmsSystems}">--%>
                            <%--<li>--%>
                                <%--<a class="waves-effect switch-systems" href="javascript:;"--%>
                                   <%--systemid="${upmsSystem.systemId}" systemname="${upmsSystem.name}"--%>
    ...
    <aside id="sidebar">
        ...
        <ul class="main-menu">
            ...
            <c:forEach var="permission" items="${permissions}" varStatus="status">
                <c:if test="${permission.pPermissionId == 0}">
                    <li class="sub-menu system_menus ${status.index}">
                        <a class="waves-effect" href="javascript:;"><i
                                class="${permission.permissionIcon}"></i> ${permission.permissionName}</a>
                        <ul>
                            <c:forEach var="subUpmsPermission" items="${permissions}">
                                <c:if test="${subUpmsPermission.pPermissionId == permission.permissionId}">
                                    <li><a class="waves-effect"
                                           href="javascript:Tab.addTab('${subUpmsPermission.permissionName}', '${basePath}${subUpmsPermission.permissionUri}');">${subUpmsPermission.permissionName}</a>
                                    </li>
                                </c:if>
                            </c:forEach>
                        </ul>
